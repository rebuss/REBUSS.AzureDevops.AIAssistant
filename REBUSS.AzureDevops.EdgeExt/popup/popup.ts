...
document.getElementById("summarizePr")?.addEventListener("click", summarizePr);
document.getElementById("reviewPr")?.addEventListener("click", reviewPr);
// document.getElementById("reviewFile")?.addEventListener("click", reviewFile);
document.getElementById("getDiff")?.addEventListener("click", getDiff);
...
function enableDevOpsButtons() {
    document.querySelectorAll<HTMLButtonElement>(".disable-on-no-devops").forEach((btn) => {
        btn.disabled = false;
        btn.style.opacity = "1";
...
function sendRequestToLocalAPI(route: string, isFile: boolean = false) {
    ...
    fetch(apiUrl, {
        ...
        .then(data => {
            if (isFile) {
                ...
                const responseText = document.getElementById("responseText") as HTMLTextAreaElement;
                if (responseText) {
